...
-- M-= / M-- to change nvim-tree width
vim.api.nvim_create_autocmd('FileType', {
  pattern = 'NvimTree',
  callback = function(ev)
    vim.keymap.set('n', '<M-=>', '<cmd>NvimTreeResize +5<CR>', { buffer = ev.buf, silent = true, desc = 'NvimTree: wider' })
    vim.keymap.set('n', '<M-->', '<cmd>NvimTreeResize -5<CR>', { buffer = ev.buf, silent = true, desc = 'NvimTree: narrower' })
  end,
})

-- https://github.com/nvim-tree/nvim-tree.lua/discussions/1672
...
end, { desc = 'NvimTree: focus or toggle' })
...
return {
  ...
  keys = {
    { mode = 'n', '<M-n>', '<cmd>NvimTreeToggle<CR>', desc = 'NvimTree: toggle window' },
    --    { mode = 'n', '<M-i>', '<cmd>NvimTreeFocus<CR>', desc = 'NvimTree: focus' },
  },
  config = function()
